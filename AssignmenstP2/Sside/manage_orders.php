    ...
    <div class="form-card" style="padding: 15px 20px; margin-bottom: 25px;">
      <div class="form-row" style="align-items: center; margin: 0; gap: 15px; flex-wrap: wrap;">
        <div class="form-group" style="margin: 0; flex: 2; min-width: 250px;">
          ...
          <input type="text" id="order-search-input" placeholder="Enter Order ID, Name, Tel, or Furniture Item..."
            onkeyup="filterAndSortOrders()">
        </div>
        ...
        <tbody>
          <?php if ($result && mysqli_num_rows($result) > 0): ?>
            <?php while ($row = mysqli_fetch_assoc($result)):
              ...
              ?>
              <tr class="order-data-row" data-oid="<?php echo $row['oid']; ?>"
                ...
                data-ostatus="<?php echo $row['ostatus']; ?>" data-items='<?php echo json_encode($items); ?>'
                data-furnitures="<?php echo htmlspecialchars($furniture_names_string, ENT_QUOTES, 'UTF-8'); ?>"
                data-timestamp="<?php echo strtotime($row['odate']); ?>" data-amount="<?php echo $row['ototalamount']; ?>"
                onclick="handleRowClick(this)" style="cursor: pointer;">
                ...
                <td>
                  <span class="badge status-<?php echo $row['ostatus']; ?>">
                    ...
                    elseif ($row['ostatus'] == 5)
                      ...
                      echo "❌ Cancelled/Rejected";
                    ?>
                  </span>
                </td>
                <td>
                  <form method="POST" action="manage_orders.php"
                    style="display:inline-flex; gap:6px; align-items:center; margin:0;" onclick="event.stopPropagation();">
                    ...
                    <div class="status-wrapper" style="padding: 4px 8px; font-size:13px;">
                      <select name="ostatus" onchange="this.form.submit()">
                        <option value="2" <?php if ($row['ostatus'] == 2)
                          echo 'selected'; ?>>⏳ Processing</option>
                        <option value="3" <?php if ($row['ostatus'] == 3)
                          echo 'selected'; ?>>✅ Approved</option>
                        <option value="4" <?php if ($row['ostatus'] == 4)
                          echo 'selected'; ?>>🚚 Out for Delivery</option>
                        <option value="5" <?php if ($row['ostatus'] == 5)
                          echo 'selected'; ?>>📦 Completed</option>
                        <option value="0" <?php if ($row['ostatus'] == 0)
                          echo 'selected'; ?>>❌ Cancel/Reject</option>
                      </select>
                    </div>
                  </form>
                </td>
          ...
          <?php else: ?>
            <tr>
              <td colspan="7" style="text-align:center; color:#7f8c8d; padding:30px;">No operational customer orders
                records logged yet inside storage.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
        ...
        <div style="padding: 10px 5px;">
          <div class="timeline-container" id="modal-timeline">
            ...
            <div class="timeline-step" id="step-4">
              <div class="timeline-icon">🚚</div>
              <div class="timeline-text">On Delivery</div>
            </div>
            ...
            <div class="timeline-step" id="step-0">
              <div class="timeline-icon">❌</div>
              <div class="timeline-text">Cancelled</div>
            </div>
          </div>
          <table
            style="width:100%; margin-bottom: 20px; font-size: 14px; background: #fdfdfd; padding: 15px; border-radius: 8px; border: 1px solid #f0f0f0;">
            <tr>
              <td style="width:25%; padding:6px 0; color:#7f8c8d;">Customer Name:</td>
              <td style="padding:6px 0; font-weight:bold;" id="modal-cname"></td>
              <td style="width:25%; padding:6px 0; color:#7f8c8d;">Order Date:</td>
              <td style="padding:6px 0;" id="modal-odate"></td>
            </tr>
            <tr>
              <td style="padding:6px 0; color:#7f8c8d;">Contact Phone:</td>
              <td style="padding:6px 0; font-weight:bold; color:#34495e;" id="modal-ctel"></td>
              <td style="padding:6px 0; color:#7f8c8d;">Est. Delivery:</td>
              <td style="padding:6px 0; font-weight:bold; color:#e67e22;" id="modal-ddate"></td>
            </tr>
            <tr>
              <td style="padding:6px 0; color:#7f8c8d;">Delivery Address:</td>
              <td style="padding:6px 0;" id="modal-caddr" colspan="3"></td>
            </tr>
            <tr>
              <td style="padding:6px 0; color:#7f8c8d;">Pipeline Management:</td>
              <td style="padding:6px 0;" colspan="3">
                <select name="ostatus" id="modal-status-select"
                  style="padding:6px 12px; border-radius:4px; border:1px solid #cbd5e1; font-weight:bold;">
          ...
          <div style="border-top: 1px solid #eee; padding-top: 15px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
              ...
              <button type="button" id="edit-items-btn" class="btn"
                style="padding:5px 12px; font-size:12px; background:#34495e; color:white; border:none; border-radius:4px;"
                onclick="toggleEditMode()">Edit Items</button>
            </div>
            <div id="modal-items-container"
              style="max-height: 280px; overflow-y: auto; margin-bottom:20px; padding-right:5px;"></div>
            <div style="background: #f1f5f9; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
              ...
              <div id="modal-total-materials-summary"
                style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; font-size: 12px;"></div>
            </div>
            <div style="border-top:1px solid #eee; padding-top:12px; font-size:14px; color:#475569;">
              <div style="display:flex; justify-content: flex-end; margin-bottom:4px;"><span
                  style="width:150px; text-align:right;">Items Subtotal:</span><span id="modal-subtotal-price"
                  style="width:100px; text-align:right; font-weight:bold;">$0.00</span></div>
              <div style="display:flex; justify-content: flex-end; margin-bottom:4px;"><span
                  style="width:150px; text-align:right;">Shipping & Handling:</span><span
                  style="width:100px; text-align:right; color:#2ecc71; font-weight:bold;">FREE</span></div>
              <div
                style="display:flex; justify-content: flex-end; font-size:18px; font-weight:bold; color:#b91c1c; margin-top:6px; border-top:1px dashed #eee; padding-top:6px;">
                <span style="width:150px; text-align:right;">Grand Total:</span><span id="modal-total"
                  style="width:100px; text-align:right;">$0.00</span></div>
            </div>
          </div>
        </div>
        <div style="margin-top: 20px; text-align: right; border-top: 1px solid #eee; padding-top:15px;">
          <button type="button" class="btn"
            style="background:#eee; color:#333; padding: 8px 20px; border:none; border-radius:4px;"
            onclick="closeOrderModal()">Cancel</button>
          <button type="submit" name="update_order_items" class="btn btn-primary"
            style="padding: 8px 25px; border:none; border-radius:4px; background:#3498db; color:white;">💾 Save Order
            Changes</button>
        </div>
      </form>
    </div>
